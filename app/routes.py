...
def explore():
    ...
    return render_template('index.html',title='Explore',posts=posts.items, next = next_url, prev=prev_url)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Congratulations for registering!')
...
def user(username):
    ...
    form = EmptyForm() # passed for follow/unfollow
